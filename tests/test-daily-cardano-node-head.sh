...
# Observe the complete suite once at the kernel-egress boundary. The inner
# run cannot skip this check: only this wrapper sets the private marker.
if [ "${HEAD_CANDIDATE_EXECVE_INNER:-}" != 1 ]; then
  strace_command=$(command -v strace || true)
  [ -n "$strace_command" ] && [ -x "$strace_command" ] || {
    printf 'FAIL: required kernel-egress observer is unavailable: strace\n' >&2
    exit 1
  }
  trace_root=$(mktemp -d)
  trap 'rm -rf -- "$trace_root"' EXIT
  trace_log="$trace_root/egress.log"
  inner_rc=0
  HEAD_CANDIDATE_EXECVE_INNER=1 "$strace_command" \
    -f -qq -s 4096 -e trace=execve,network -e signal=none -o "$trace_log" \
    "$repo_root/tests/test-daily-cardano-node-head.sh" || inner_rc=$?
  [ "$inner_rc" -eq 0 ] || exit "$inner_rc"

  unfinished_execve_pids=$(sed -n 's/^\([0-9][0-9]*\)  *execve(.* <unfinished \.\.\.>$/\1/p' "$trace_log" | sort)
  resumed_execve_pids=$(sed -n 's/^\([0-9][0-9]*\)  *<\.\.\. execve resumed>.*$/\1/p' "$trace_log" | sort)
  [ "$unfinished_execve_pids" = "$resumed_execve_pids" ] || {
    printf 'FAIL: observer trace contains an unmatched split execve record\n' >&2
    exit 1
  }
  execve_record_pattern='^[0-9]+  *(execve\(|<\.\.\. execve resumed>)'
  unlisted_records=$(grep -Evc "$execve_record_pattern" "$trace_log" || true)
  [ "$unlisted_records" -eq 0 ] || {
    unlisted_record=$(grep -Ev "$execve_record_pattern" "$trace_log" | sed -n '1p')
    printf 'FAIL: observer found %s unlisted kernel-egress records; first: %s\n' \
      "$unlisted_records" "$unlisted_record" >&2
    exit 1
  }

  mapfile -t executed_paths < <(
    sed -n -e 's/^[0-9][0-9]*  *execve("\([^"]*\)".*) = 0$/\1/p' \
      -e 's/^[0-9][0-9]*  *execve("\([^"]*\)".* <unfinished \.\.\.>$/\1/p' \
      "$trace_log"
  )
  successful_execves=$(grep -Ec '(execve\(.*\) = 0$|<\.\.\. execve resumed>\).* = 0$)' "$trace_log" || true)
  [ "${#executed_paths[@]}" -eq "$successful_execves" ] || {
    printf 'FAIL: execve observer parsed %s of %s successful records\n' \
      "${#executed_paths[@]}" "$successful_execves" >&2
    exit 1
  }
  [ "${#executed_paths[@]}" -gt 0 ] || {
    printf 'FAIL: execve observer produced no successful records\n' >&2
    exit 1
  }

  allowed_paths=(
    "$repo_root/scripts/daily-cardano-node-head.sh"
    "$repo_root/tests/fixtures/daily-cardano-node-head/fake-transport.sh"
    "$repo_root/tests/test-daily-cardano-node-head.sh"
  )
  for allowed_command in bash dirname mkdir sed awk grep sort env mktemp rm chmod; do
    allowed_path=$(command -v -- "$allowed_command" || true)
    [ -n "$allowed_path" ] && [ -x "$allowed_path" ] || {
      printf 'FAIL: allowlisted command is unavailable: %s\n' "$allowed_command" >&2
      exit 1
    }
    allowed_paths+=("$allowed_path")
  done
  observed_programs=$(printf '%s\n' "${executed_paths[@]}" | sort -u)
  allowed_programs=$(printf '%s\n' "${allowed_paths[@]}" | sort -u)
  [ "$observed_programs" = "$allowed_programs" ] || {
    printf 'FAIL: executed-program set differs from allowlist\nallowed:\n%s\nobserved:\n%s\n' \
      "$allowed_programs" "$observed_programs" >&2
    exit 1
  }
  printf 'PASS egress-allowlist execve=%s network=0 allowed=%s\n' \
    "${#executed_paths[@]}" "${#allowed_paths[@]}"
  exit 0
fi
